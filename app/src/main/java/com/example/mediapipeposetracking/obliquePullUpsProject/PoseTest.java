...
public class PoseTest {
    ...
    public boolean[] isPoseCorrect(Point LShoulder,Point LHip,Point LKeen,Point LAnkle,Point LHeel,Point LIndex,
        ...
        boolean feet_moved;
        //首先检测腿是否伸直
        boolean Lleg=BodyModule.isLegStright(LHip,LKeen,LAnkle, threshold_leg_angle_sub);
        boolean Rleg=BodyModule.isLegStright(RHip,RKeen,RAnkle, threshold_leg_angle_sub);
        //当两条腿都不直时，提醒腿部弯曲
        if(!Lleg&&!Rleg){
            leg_bend=true;
//            System.out.println("请伸直双腿");
        ...
        //其次检测身体是否笔直
        boolean Lbody=BodyModule.isBodyStright(LShoulder,LHip,LKeen,LAnkle, threshold_body_leg_angle_sub);
        boolean Rbody=BodyModule.isBodyStright(RShoulder,RHip,RKeen,RAnkle, threshold_body_leg_angle_sub);
        //当两边身体都不直时，提醒弯曲
        if(!Lbody&&!Rbody){
            body_bend=true;
//            System.out.println("请挺直身体");
    ...
    public boolean isStartPose(Point LShoulder,Point LWrist,Point LElbow,Point LAnkle,
        ...
        System.out.println("左右胳膊-身体角度阈值："+this.LArmBodyAngle+"  "+this.RArmBodyAngle);
        ...
        System.out.println("左右身体角度阈值："+upperBoundLeftBody+"  "+upperBoundRightBody);
...
//
        ...
        System.out.println("stright,angle,anglefloor boolean？  "+armStrightFlag+" "+armAngleFlag+" "+bodyFlag);
        ...
        //基本要求,满足左右其中之一即可
        boolean commonConditionLeft=(angle_body_arm_left>75 &&angle_body_arm_left<105) &&
                (angle_arm_left<30 )&& (angle_body_floor_left<60);

        boolean commonConditionRight=(angle_body_arm_right>75 &&angle_body_arm_right<105) &&
                (angle_arm_right<30 )&& (angle_body_floor_right<60);

        boolean commonCondition=commonConditionLeft||commonConditionRight;
    ...
    public boolean isReady(Point LShoulder,Point LWrist,Point LElbow,Point LAnkle,
                           Point RShoulder,Point RWrist,Point RElbow,Point RAnkle){
        //胳膊伸直 tag1
        boolean isArmStright_left = ArmModule.isArmStright(LWrist, LElbow, LShoulder, 20);
        boolean isArmStright_right = ArmModule.isArmStright(RWrist, RElbow, RShoulder, 20);
        boolean armStrightFlag=(isArmStright_left && isArmStright_right);
        ...
        System.out.println("ready arm:"+angle_body_arm_left+" "+angle_body_arm_right);
        ...
        if (armStrightFlag && armAngleFlag && bodyFlag){
            return true;
        }
    ...
    public boolean satisfyCondition(Point LWrist,Point LElbow,Point LShoulder, Point LAnkle, Point RWrist,Point RElbow,Point RShoulder, Point RAnkle,Point Nose){
        ...
        if(sub_angle_left<PoseTest.threshold_angle_sub ||sub_angle_right<PoseTest.threshold_angle_sub){
            ...
            if (condition_circumference_satisfy||condition_dis_nose_satisfy){
                return true;
            }
        }
        return false;
